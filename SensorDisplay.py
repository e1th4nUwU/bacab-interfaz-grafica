...
class SensorDisplay(ctk.CTkFrame):
    ...
    def setup_widgets(self):
        ...
        for i in range(no_filas):
            self.rowconfigure(i, weight=1)  # Hace que la primera fila crezca con la ventana
        for j in range(no_columnas):
            self.columnconfigure(j, weight=1)  # Hace que la primera columna crezca con la ventana

        # Initialize labels and values for each sensor
        # Temperatura
        ...
        # Orientacion
        self.label_oritentacion_label = ctk.CTkLabel(self, text="Orientación", font=font_default)
        self.label_oritentacion_label.grid(row=0, column=2, padx=10)
        ...
        # Aceleracion
        self.label_temp7 = ctk.CTkLabel(self, text="Aceleración", font=font_default)
        self.label_temp7.grid(row=0, column=3, padx=10)

        # Aceleracion X
        self.value_aceleracionx = ctk.CTkLabel(self, text="0 °", font=font_data, fg_color="#535862", corner_radius=5)
        self.value_aceleracionx.grid(row=2, column=3, padx=10)

        # Aceleracion Y
        self.value_aceleraciony = ctk.CTkLabel(self, text="0 °", font=font_data, fg_color="#535862", corner_radius=5)
        self.value_aceleraciony.grid(row=4, column=3, padx=10)

        # Aceleracion Z
        self.value_aceleracionz = ctk.CTkLabel(self, text="0 °", font=font_data, fg_color="#535862", corner_radius=5)
        self.value_aceleracionz.grid(row=6, column=3, padx=10)

    def update_sensor_data(self, new_data):
        # new_data es una lista con la siguiente estructura:
        # [temperatura, presion, latitud, longitud, ...]
        ...
        self.value_orientacionx.configure(text=f"{new_data[6]} m")
        self.value_orientaciony.configure(text=f"{new_data[7]} m")
        self.value_orientacionz.configure(text=f"{new_data[8]} m")
        ...
        # Y así sucesivamente para el resto de los sensores...

# Uso de la clase SensorDisplay y la función update_sensor_data
# sensor_display = SensorDisplay(master)
# Continuamente en algún loop o después de leer datos:
# sensor_display.update_sensor_data(datos_del_arduino)
5